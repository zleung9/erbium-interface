...
class Distance():
    ...
    def from_parent(cls, p):
        ...
        """
        return cls(p.name, p.atom_id, p.data)
...
class ErSPCDistance(Distance):
    ...
    def get_1st_shell_type(self, threshold=3.0, binding_frame=None):
        is_1st_shell = (self.data <= threshold)
        if is_1st_shell.sum() == 0: # Water never goes into 1st shell
            ...
            if binding_frame is not None:
                ...
                if (self.data[select] <= threshold).sum() > 0:
...
class ErPDistance(Distance):
    ...
    def get_binding_info(self, threshold=4.3):
        self.binding_frame = (self.data >= threshold).sum()
        return self.binding_frame


class Distances():
    def __init__(self, atom_pairs, distance_list):
        # A list of the indices of atoms for which the distance is calculated.
        self.pairs = [tuple(pair) for pair in atom_pairs]
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, int):
            return self.distance_list[item]
        elif isinstance(item, str):
            raise NotImplementedError
        elif isinstance(item, tuple):
            return self.distance_list[self.pairs.index(item)]

    def __len__(self):
        return len(self.distance_list)

    @classmethod
    def from_parent(cls, p, child_cls):
        """ Construct an child_cls object from an existing Distances instance `p`.
        The `child_cls` inherits from Distance.
        """
        distance_list = [child_cls.from_parent(distance) for distance in p]
        return cls(p.pairs, distance_list)
    ...
    def to_numpy(self, start=None, end=None, smooth=0):
        ...
        data =  np.stack(
            [distance.data[start:end] for distance in self.distance_list]
        )
...
class ErSPCDistances(Distances):
    ...
    def __init__(self, atom_pairs, distance_list):
        super().__init__(atom_pairs=atom_pairs, distance_list=distance_list)
    ...
    def get_1st_shell_upon_binding(self, binding_frame):
        ...
        return ErSPCDistances(pairs_1st_shell, distances_1st_shell)
